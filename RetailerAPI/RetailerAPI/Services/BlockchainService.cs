...
namespace RetailerAPI.Services
{
    public class BlockchainOptions
    {
        ...
        public string ActorName { get; set; } = "Retailer_C";
    ...
    public class BlockchainService
    {
        ...
        public async Task<bool> ReceiveRetailAsync(ReceiveRetailDTO dto)
        {
            ...
            if (_options.EnableCrypto)
            {
                try
                {
                    cryptoToUse = new CryptoHelper(dto.RetailerName, "keys", _logger);
                    if (!cryptoToUse.KeysExist())
                    {
                        _logger.LogError("Keys not found for {Actor}", dto.RetailerName);
                        return false;
                    }
                }
                catch (Exception ex)
                {
                    _logger.LogError(ex, "Failed to load crypto for {Actor}", dto.RetailerName);
                    return false;
                }
            }
            ...
            var payload = new
            {
                ...
                action = "received_retail",
                actor = dto.RetailerName,
                metadata = new { from = dto.DistributorName, to = dto.RetailerName }
            };

            return await SendTransactionAsync(payload, cryptoToUse);
        }

        public async Task<bool> SoldAsync(SaleDTO dto)
        {
            CryptoHelper? cryptoToUse = null;
            ...
            var payload = new
            {
                ...
                action = "sold",
                actor = dto.RetailerName,
                metadata = new { customer = dto.CustomerName, date = dto.SaleDate }
            };

            return await SendTransactionAsync(payload, cryptoToUse);
        }

        public async Task<object?> VerifyAsync(string batchId)
        {
            // Use GetHistoryAsync which has failover built-in
            return await GetHistoryAsync(batchId);
        }
