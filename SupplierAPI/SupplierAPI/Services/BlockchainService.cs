...
namespace SupplierAPI.Services
{
    public class BlockchainOptions
    {
        ...
        public string ActorName { get; set; } = "Supplier_A";
    ...
    public class BlockchainService
    {
        ...
        public async Task<bool> AddProductAsync(ProductDTO dto)
        {
            ...
            if (_options.EnableCrypto)
            {
                try
                {
                    cryptoToUse = new CryptoHelper(dto.Supplier, "keys", _logger);
                    if (!cryptoToUse.KeysExist())
                    {
                        _logger.LogError("Keys not found for {Actor}", dto.Supplier);
                        return false;
                    }
                }
                catch (Exception ex)
                {
                    _logger.LogError(ex, "Failed to load crypto for {Actor}", dto.Supplier);
                    return false;
                }
            }
            ...
            var payload = new
            {
                ...
                action = "registered",
                actor = dto.Supplier,
                metadata = new { product = dto.ProductName, quantity = dto.Quantity }
            };

            return await SendTransactionAsync(payload, cryptoToUse);
        }

        public async Task<bool> QualityCheckAsync(QualityCheckDTO dto)
        {
            CryptoHelper? cryptoToUse = null;
            ...
            if (_options.EnableCrypto)
            {
                try
                {
                    cryptoToUse = new CryptoHelper(dto.SupplierName, "keys", _logger);
                    if (!cryptoToUse.KeysExist())
                    {
                        _logger.LogError("Keys not found for {Actor}", dto.SupplierName);
                        return false;
                    }
                }
                catch (Exception ex)
                {
                    _logger.LogError(ex, "Failed to load crypto for {Actor}", dto.SupplierName);
                    return false;
                }
            }
            ...
            var payload = new
            {
                ...
                actor = dto.SupplierName,
                metadata = new { result = dto.Result, inspector = dto.Inspector }
            };

            return await SendTransactionAsync(payload, cryptoToUse);
        }

        public async Task<bool> ShipAsync(ShipDTO dto)
        {
            CryptoHelper? cryptoToUse = null;
            ...
            var payload = new
            {
                ...
                action = "shipped",
                actor = dto.SupplierName,
                metadata = new { from = dto.SupplierName, to = dto.DistributorName }
            };
        ...
        private async Task<bool> SendTransactionAsync(object payload, CryptoHelper? cryptoOverride = null)
        {
            ...
            for (int nodeAttempt = 0; nodeAttempt < _options.NodeUrls.Count; nodeAttempt++)
            {
                try
                {
                    ...
                    {
                        var errorContent = await response.Content.ReadAsStringAsync();
                        _logger.LogWarning("Transaction already exists: {Error}", errorContent);
                        return true;
                    }
